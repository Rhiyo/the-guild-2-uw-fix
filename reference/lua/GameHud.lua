function Init()
	...
	-- this:AddPanel("TestRender","cl_StaticPanel","gui/hud/testrender.gui",false)
   ...
   -- InGame Menus
	...
	-- Ultrawide fix: move the right-edge HUD panels to the physical right edge.
	-- Walk \GUI\HudRoot, match by PanelName, set ABS_X/ABS_Y on the GUI node
	-- (the modpack's Minimap.lua pattern; the node's X lives at [node+0x64] and
	-- ABS_X is plumbed to it by shared base code -- verified in GuildII.exe).
	--
	-- CharactersPanel test: static RE of cl_CharactersPanel (all 70 vtable
	-- methods) found NO code that computes the panel's own X from the canvas
	-- width, so it should honor ABS_X exactly like cl_ButtonPanel. We log every
	-- panel's name + current rect so we can confirm the real node name and
	-- whether the write sticks.
	-- nil-safe stringifier: this Lua sandbox does NOT expose global tostring.
	-- numbers coerce in '..' fine; only nil concatenation errors, so guard it.
	local function sv(v)
	...
	if gfx ~= nil and root ~= nil then
		...
		LogMessage("@ULTRAWIDE screen sw="..sv(sw).." sh="..sv(sh))
		if sw ~= nil and sw > 0 then
			local cnt = root:GetChildCnt()
			LogMessage("@ULTRAWIDE HudRoot childcnt="..sv(cnt))
			for i = 0, cnt - 1 do
				local child = root:GetChildAt(i)
				if child ~= nil then
					local pn = child:GetValueString("PanelName")
					local cx = child:GetValueInt("ABS_X")
					local cy = child:GetValueInt("ABS_Y")
					local cw = child:GetValueInt("ABS_WIDTH")
					LogMessage("@ULTRAWIDE panel["..i.."] name="..sv(pn)
						.." ABS_X="..sv(cx).." ABS_Y="..sv(cy)
						.." ABS_WIDTH="..sv(cw))
					...
					elseif pn == "CharactersPanel" then
						-- top-right: push to physical right edge, keep current Y
						local w = cw
						if w == nil or w <= 0 then w = 240 end
						child:SetValueInt("ABS_X", sw - w)
						local back = child:GetValueInt("ABS_X")
						LogMessage("@ULTRAWIDE CharactersPanel -> set ABS_X="
							..(sw - w).." readback="..sv(back))
					end
				end
			end
		end
